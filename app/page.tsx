...
const onSubmit = async (data: registrationForms) => {
  try {
    ...
    formData.append("contactNumber", String(data.contactNumber));
    formData.append("chapter", data.chapter);
    formData.append("file", data.receipt[0]); // actual file
    ...
    if (response.ok) {
      ...
      alert(resData.message || "Error registering.");
    }
  } catch (err) {
    console.error(err);
